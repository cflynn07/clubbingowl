...
jQuery(function(){
	...
	window.vc_page_scripts.admin_promoter_manage_image = function(){
		...
		var initialize_ocupload = function(){
	        ...
	        myUpload = jQuery('#ocupload_button').upload({
		        ...
		        onComplete: function(response){
		        	...
		        	jQuery('img#image').attr('src', window.module.Globals.prototype.s3_uploaded_images_base_url + ((window.page_obj.manage_image.type == 'guest_lists') ? 'guest_lists' : 'events') + '/originals/' + ((window.page_obj.manage_image.live_image) ? '' : 'temp/') + response.image_data.image + '.jpg');
		...
		var initialize_crop = function(){
			...
			crop_object = jQuery('img#image').imgAreaSelect({
				instance: true,
				handles: true,
				show: true,
				persistent: true,
