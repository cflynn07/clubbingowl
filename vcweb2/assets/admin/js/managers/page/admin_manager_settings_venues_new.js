...
jQuery(function(){
	...
	window.vc_page_scripts.admin_manager_settings_venues_new = function(){
		...
		venue_guest_lists = jQuery.cookies.get('venue_guest_lists') || [];
		...
		var build_guest_lists_table = function(){
			...
			for(i in venue_guest_lists){
				console.log(venue_guest_lists[i]);
				
				var weekday;
				switch(venue_guest_lists[i].weekday){
					case '0':
						weekday = 'Mondays';
						break;
					case '1':
				...
				table_html += '	<td>' + venue_guest_lists[i].name + '</td>'; //
				table_html += '	<td>' + weekday + '</td>'; //weekday
				table_html += '	<td>' + venue_guest_lists[i].description + '</td>'; //description
				table_html += '	<td><input type="checkbox" class="iphone" name="list_auto_approve" ' + ((venue_guest_lists[i].auto_approve) ? 'checked="checked"' : '') + ' /></td>'; //auto-approve			
