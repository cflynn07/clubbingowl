...
<fieldset>
		<legend>Upload and Crop <?= ($manage_image->type == 'venues') ? 'Venue' : '' ?> Image</legend> 
		
		<img id="image" src="<?= (isset($manage_image->image_data)) ? $central->s3_uploaded_images_base_url . $manage_image->type . '/originals/' . ((!$manage_image->existing) ? 'temp/' : '') . $manage_image->image_data->image . '.jpg' : '' ?>" alt="<?= ($manage_image->type == 'venues') ? 'Venue' : '' ?> Image" style="display:none;" />
...
<div style="display:none;">
	<a class="ajaxify" id="back" href="<?= $central->front_link_base . 'admin/managers/' . $manage_image->return . '/' ?>">Back</a>
</div>