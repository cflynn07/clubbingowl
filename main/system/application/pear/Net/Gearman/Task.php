...
class Net_Gearman_Task
{
    ...
    public function __construct($options) 
    {
		...
		// ---- adjustments to make compatable with CI --------
		if(is_array($options)){
			
			if(array_key_exists('func', $options) && array_key_exists('arg', $options)){
				...
				if(array_key_exists('uniq', $options))
					$uniq = $options['uniq'];
				...
				die('Invalid argument, Array missing keys: Net/Gearman/Task');
			}
			
		}else{
			die('Invalid argument: Net/Gearman/Task');
		}
		// -----------------------------------------------------
