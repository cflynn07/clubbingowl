...
jQuery(function(){
	...
	window.vc_page_scripts.promoter_guest_list_individual = function(){
		...
		var pgla_id 			= gl_obj.pgla_id;
		var up_id				= gl_obj.up_id;
		var promoter_first_name = gl_obj.promoter_first_name;
		...
		window.promoters_guest_list = {
			invitees: []
		}
		
		var friend_invite_callback = function(){
			...
			FB.ui({
				...
				message: vc_user.first_name + ' has added you to their entourage for ' + promoter_first_name + '\'s guest list with ClubbingOwl!',
				data: JSON.stringify(data),
				exclude_ids: promoters_guest_list.invitees
			}, function(request){
				...
				if(request.to.length > 0){
					...
					if(promoters_guest_list.invitees.length === 0)
					...
					query.wait(function(rows){
						...
						for(var i in rows){
							...
							promoters_guest_list.invitees.push(rows[i].uid);
						}
		...
		jQuery('a.facebook-invite').bind('click', friend_invite_callback);
		unload_items.push([
			'click',
			jQuery('a.facebook-invite'),
			friend_invite_callback
		]);
		
		var gl_submit_callback = function(e){
			...
			var data = {
				ci_csrf_token: 		cct,
				vc_method: 			'promoter_guest_list_join_request',
				gl_id: 				pgla_id,
				entourage: 			promoters_guest_list.invitees,
		...
		jQuery('input#guestlist-form-submit').bind('click', gl_submit_callback);
		unload_items.push([
			'click',
		...
		jQuery("textarea#guestlist-message").keyup(keyup_callback);
		unload_items.push([
			'keyup',
		...
		var rm_func = function(){
			...
			window.promoters_guest_list.invitees = [];
