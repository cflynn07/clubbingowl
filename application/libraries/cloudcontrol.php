...
class Cloudcontrol {
     ...
     * @return API
     */
    public function __construct($url=Cloudcontrol::URL) {
        $this->setUrl($url);
    }
    ...
    public function checkToken($token) {
        if (strlen($token) == Cloudcontrol::TOKEN_STRLEN) {
            return true;
        }
        return false;
    }
...
class Request {
     ...
     * @param string $version client-version [default:Cloudcontrol::VERSION]
     */
    public function __construct($url, $token=null, $version=Cloudcontrol::VERSION) {
        $this->_url = $url;
        $this->_token = $token;
    ...
    private function _request($resource, $method=HTTP_Request2::METHOD_GET, $data=array(), $headers=array()) {
        ...
        $request->setConfig('ssl_verify_peer', Cloudcontrol::SSL_VERIFY_PEER);
